...
<div class="content">
    ...
    $totalExpenses = $pdo->query("SELECT SUM(amount) FROM expenses")->fetchColumn(); // Fetch total expenses
    ?>
    ...
    <h2>Recent Orders</h2>
    <?php
    $orders = $pdo->query("SELECT orders.id, customers.name AS customer_name, orders.service, orders.weight, orders.created_at FROM orders JOIN customers ON orders.customer_id = customers.id ORDER BY orders.created_at DESC LIMIT 5")->fetchAll(PDO::FETCH_ASSOC);
    ?>
